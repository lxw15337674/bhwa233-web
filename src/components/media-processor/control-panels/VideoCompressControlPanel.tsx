...
export const VideoCompressControlPanel: React.FC<ControlPanelProps> = ({
  ...
  // 文件验证
  const fileValidation = selectedFile ? validateVideoFile(selectedFile) : { valid: true };

  // 压缩预估
  const compressionEstimate = selectedFile ? estimateVideoCompression(
    mediaMetadata,
    selectedFile,
    outputFormat,
    resolution
  ...
  const handleStartProcessing = async () => {
    ...
    try {
      ...
      const outputFileName = `${selectedFile.name.replace(/\.[^/.]+$/, '')}_compressed.${VIDEO_FORMATS[outputFormat].ext}`;
      ...
      <Card className="bg-card border-border">
        <CardContent className="p-4">
          <div className="space-y-4">
            ...
            {/* 压缩预估 */}
            {selectedFile && compressionEstimate && fileValidation.valid && (
            ...
            {/* 无视频信息时的提示 */}
            {!isAnalyzing && !mediaMetadata && !analyzeError && selectedFile && fileValidation.valid && (
