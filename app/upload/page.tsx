    ...
    const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!validation.valid) {
            ...
            if (validation.validFiles.length > 0) {
                setErrorMsg(`以下文件超过10MB限制已被过滤：${oversizedInfo}。其余${validation.validFiles.length}个文件可正常上传。`);
            } else {
                setErrorMsg(`所有文件都超过10MB限制：${oversizedInfo}`);
                return;
            }
        } else {
            setErrorMsg(null);
        ...
        <div className="container mx-auto px-4 py-8 max-w-2xl">
            ...
            <header className="text-center mb-8">
                ...
                <div className="flex flex-wrap justify-center gap-2 text-xs">
                    ...
                    <span className="bg-purple-100 text-purple-800 px-2 py-1 rounded">最大10MB</span>
                        ...
                        {uploadFiles.length > 0 ? (
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-lg font-medium">拖拽文件到这里或点击选择</p>
                                    <p className="text-sm text-muted-foreground">支持任意格式文件，单个文件最大10MB</p>
                            </div>
                        )}
                        <input
                            id="file-input"
